...
test("scripted route flow: first meeting, naming, transfer, and separate plushies", async (t) => {
  ...
  answers.tap = "OWNER";
  const claimJar = [first.cookie, "celebrate=named"].join("; ");
  const owner = await request(tapUrl(), { cookie: claimJar });
  pages.push(owner.html);
  assert.match(owner.html, /만나서 반가워요, Mochi!/);
  assert.doesNotMatch(owner.html, /다시 만나서 반가워요/);
  assert.match(owner.html, /data-tap-count/);
  assert.match(owner.html, /우리 2번 토닥였어요!/);
  assert.match(owner.html, /data-celebrate="named"/);
...
test("claim celebrate flash is server-consumed and missing on the next tap", async (t) => {
  ...
  assert.match(named.setCookies.join("\n"), /celebrate=named/);
  answers.tap = "OWNER";
  const celebrated = await request(tapUrl(), { cookie: `${first.cookie}; celebrate=named` });
  assert.match(celebrated.html, /data-celebrate="named"/);
...
test("dev preview celebration routes render and stay hidden in production", async (t) => {
  ...
  assert.match(claimPreview.html, /만나서 반가워요, 미리보기!/);
  assert.doesNotMatch(claimPreview.html, /다시 만나서 반가워요/);
  ...
  assert.match(levelup.html, /쑥쑥 컸어요! 이제 Lv\. 2!/);
  ...
  assert.match(lonely.html, /혼자 있어서 심심했어요/);
